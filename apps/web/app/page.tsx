...
const CTA_CARDS = [
  {
    ...
    tone: 'border-line bg-danger-soft hover:border-danger'
  },
  {
    title: '研究信号台',
    description: 'AI 信号流、双语摘要与置信度过滤，帮助快速筛选可行动线索。',
    href: '/research' as Route,
    tone: 'border-line bg-accent-soft hover:border-accent'
  },
  {
    title: '路径推演',
    ...
    >
      <section className="rounded-2xl border border-line bg-surface p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href="/dashboard"
            className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-surface transition hover:bg-ink"
          >
            打开决策驾驶舱
          </Link>
          <Link
            href="/scenarios"
            className="rounded-full border border-line bg-surface px-4 py-2 text-sm font-semibold text-ink transition hover:border-accent hover:bg-accent-soft hover:text-accent"
          >
            测试情景
          </Link>
          <Link
            href="/sources"
            className="rounded-full border border-line bg-surface px-4 py-2 text-sm font-semibold text-muted transition hover:border-accent hover:bg-accent-soft hover:text-accent"
          >
            查看来源质量
          </Link>
        </div>
      ...
      <section className="mt-8 rounded-2xl border border-success bg-surface p-6">
        <p className="text-xs uppercase tracking-[0.16em] text-success">双域叙事</p>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <Link
            href="/grid"
            className="rounded-full border border-success bg-surface px-4 py-2 text-sm font-semibold text-success transition hover:border-success hover:bg-success-soft hover:text-ink"
          >
            查看电网平价
          </Link>
          <Link
            href="/crisis/saf-tipping-point"
            className="rounded-full border border-accent bg-surface px-4 py-2 text-sm font-semibold text-accent transition hover:border-accent hover:bg-accent-soft hover:text-ink"
          >
            查看 SAF 转折点
          </Link>
        </div>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-2xl border border-line bg-surface p-5">
          <p className="text-xs uppercase tracking-[0.16em] text-muted">核心指标 1 · 库存覆盖</p>
          <h3 className="mt-2 text-lg font-semibold text-ink">/v1/reserves/eu</h3>
        ...
        <article className="rounded-2xl border border-line bg-surface p-5">
          <p className="text-xs uppercase tracking-[0.16em] text-muted">核心指标 2 · 转折事件</p>
          <h3 className="mt-2 text-lg font-semibold text-ink">/v1/analysis/tipping-point/events</h3>
        ...
        <article className="rounded-2xl border border-line bg-surface p-5">
          <p className="text-xs uppercase tracking-[0.16em] text-muted">核心指标 3 · 研究信号</p>
          <h3 className="mt-2 text-lg font-semibold text-ink">/v1/research/signals</h3>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {CTA_CARDS.map((card) => (
          <Link
            key={card.title}
            href={card.href}
            className={`rounded-2xl border p-5 transition ${card.tone}`}
          >
            <p className="text-xs uppercase tracking-[0.16em] text-muted">深入查看</p>
