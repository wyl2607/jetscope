...
// ---------------------------------------------------------------------------
// Reserve fallback — curated default only (NOT an IATA/EUROCONTROL live feed).
// Override weeks via SAFVSOIL_RESERVE_WEEKS. Official gap % only via env on API.
// Supply-status note as_of 2026-08-04 from Lufthansa CEO comments in article.
// ---------------------------------------------------------------------------
function getReserveData(): { weeks: number; updatedAt: string; source: string; nextUpdate: string } {
  ...
  return {
    weeks,
    updatedAt,
    source: 'JetScope curated / env override (not IATA/EUROCONTROL live feed)',
...
export default async function EuJetReserveCrisisPage() {
  ...
  const reserve = readModel.reserve
    ? {
        ...
        nextUpdate: new Date(
          new Date(readModel.reserve.generated_at).getTime() + 7 * 24 * 60 * 60 * 1000
        ).toISOString()
      }
    : fallbackReserve;
  ...
  const safHeffaLow = Number(process.env.SAF_HEFFA_LOW) || 1.60;
  const safHeffaHigh = Number(process.env.SAF_HEFFA_HIGH) || 1.85;
