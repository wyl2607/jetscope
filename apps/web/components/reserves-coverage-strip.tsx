  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-slate-400">储备信号</p>
          <h3 className="mt-2 text-xl font-semibold text-white">欧盟航煤储备覆盖</h3>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
          <p className="text-xs uppercase tracking-[0.15em] text-slate-400">覆盖</p>
          <p className={`mt-2 text-3xl font-semibold ${style.text}`}>{reserve.coverage_weeks.toFixed(2)}w</p>
          <p className="mt-1 text-xs text-slate-400">约 {reserve.coverage_days.toFixed(0)} 天</p>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
          <p className="text-xs uppercase tracking-[0.15em] text-slate-400">供给缺口压力</p>
          <p className="mt-2 text-3xl font-semibold text-white">{reserve.estimated_supply_gap_pct.toFixed(1)}%</p>
          <p className="mt-1 text-xs text-slate-400">模型推导的压力指标</p>
        </div>
      ...
      <div className="mt-6">
        ...
        <p className="mt-3 text-xs text-slate-400">更新于 {formatUpdatedAt(reserve.generated_at)} · 置信度 {(reserve.confidence_score * 100).toFixed(0)}%</p>
      </div>
    </section>
  );
}
