...
def test_get_pathway_source_normalizes_key_and_maps_maturity_confidence() -> None:
    # Pin as_of inside the annual EASA window so this asserts the fresh path.
    source = get_pathway_source(" HEFA ", as_of=date(2026, 9, 23))

    assert source["source_type"] == "official"
    assert source["confidence_score"] == pytest.approx(0.8)
    assert source["cadence"] == "annual"
    assert source["updated_at"] == "2026-02-26"
    assert source["fallback_used"] is False
    assert source["source_url"] == "https://www.easa.europa.eu/en/downloads/143282/en"
...
def test_get_pathway_source_returns_copy_not_internal_state() -> None:
    ...
    assert fresh_source["source_type"] == "official"
...
def test_list_pathway_sources_covers_cost_pathways_and_returns_copies() -> None:
    ...
    assert fresh_sources["ptl"]["cadence"] == "annual"
    assert fresh_sources["fossil_jet_crisis"]["cadence"] == "quarterly"
...
def test_stale_pathway_source_marks_fallback_and_caps_confidence() -> None:
    ...
    # EASA briefing note 2026-02-26; annual window is 400 days.
    assert get_pathway_source("hefa", as_of=date(2027, 4, 2))["fallback_used"] is False
    source = get_pathway_source("hefa", as_of=date(2027, 4, 3))
...
def test_stale_pathway_list_applies_fallback_semantics_to_all_rows() -> None:
    sources = list_pathway_sources(as_of=date(2027, 5, 1))
...
def test_stale_cap_does_not_raise_low_maturity_confidence() -> None:
    ...
    source = get_pathway_source("ptl", as_of=date(2027, 5, 1))
