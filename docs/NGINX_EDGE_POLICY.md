...
TLS server blocks under `infra/tls/` are operator state. When adding a 443 block, duplicate the same location-level headers, cache exclusions, and admin Basic Auth directives; a separate Nginx `server` block does not inherit them from the HTTP block.
