...
### 3.11 未知不许洗成 0（P3 batch 1/2 新增）

对 `Figure.value` 写 `?? 0` **一律禁止**，无论是渲染还是几何计算。

这条单独立项，是因为它在 P3 的前两批里**换着马甲出现了两次**：

- batch 1：`completeness.value ?? 0`，接口挂了页面显示「完整度 0%」；
  read model 里还配了个 `assumed({ value: 0 })` 把它坐实成情景值。
- batch 2：类型转换本身做对了，但新增了一个 helper
  ...
  把每个 Figure 在绘图层洗回裸数字，null 性在最后一米丢干净。

两次的闸口都是全绿的：`web:gate` 过、171 个测试全过、`figure-contract-lint` 也过。
**lint 判不了「这个 0 该不该是 null」**，它只看类型是不是 `Figure`。
所以这条只能靠复审，不能指望闸口。

正确做法，按场景：

- **图表/柱状**：`value` 为 null 的条目**不进入绘图集合**，在图例或脚注里显式说明
  它未知及其理由。画在 0 位比不画更误导——那是在宣称「测到了，结果是零」。
- **聚合运算**（如 low/high 取中点）：任一端为 null，结果就是未知。
  渲染 `missing()` / "—"，不要拿 0 参与运算。
- **坐标轴上限**：只用有值的条目算，别让补进去的 0 把量程拉歪。
- **文本**：走 `FigureValue`，不要 `.value!.toFixed()`。

根因不是手误，是模型的系统性倾向：算术需要 `number`，于是它本能地把 `Figure`
拆回去，而 `?? 0` 是最短的拆法。**写任务包时要把这条作为通用禁令写进去，
不要逐点列举**——逐点列举只会让它在下一个没列到的点上重犯。
