...
done < <(find "$root" \( -path "$root/Documents/Obsidian*" -o -name 'obsidian-*.md' -o -name 'obsidian-*.json' -o -name 'obsidian-*.log' -o -name 'obsidian-*.txt' -o -name 'Obsidian_*.md' \) 2>/dev/null || true)
if [ "${#hits[@]}" -gt 0 ]; then
